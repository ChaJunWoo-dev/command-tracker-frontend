...
1. 썸네일 개수 결정 문제
   ...
   화면 크기에 맞춰 썸네일 개수를 동적으로 계산하면, 컨테이너 너비를 계산해야 하고 리사이즈 대응까지 고려해야 합니다. 반면 썸네일 생성은 비디오 seek를 동반하는 비용이 큰 작업이라, 화면 크기가 바뀔 때마다 재생성하는 것은 비효율적입니다.
...
3. 메모리 사용량 문제
   ...
   고해상도 프레임을 원본 크기 그대로 썸네일에 사용하면 메모리 사용량이 급격히 증가합니다.

#### 해결 방법

1. 고정 개수 썸네일 + CSS 유연 표시

   썸네일 개수를 고정하고, 각 썸네일을 컨테이너 크기에 관계없이 균등하게 채우도록 구현했습니다.

   썸네일 생성은 비디오 seek 작업을 동반하므로 비용이 큽니다. 창 크기가 변할 때마다 재생성하면 UX가 오히려 저하된다고 판단하여, 초기 로드 시 한 번만 생성하고 CSS로 유연하게 표시하는 방식을 선택했습니다.

   > seek란, 비디오에서 특정 시점으로 이동하는 동작입니다. 예를 들어 `video.currentTime = 30`을 설정하면 브라우저가 30초 지점의 영상 데이터를 찾아 디코딩하는 과정이 발생하고, 이 과정이 완료되면 `seeked` 이벤트가 발생합니다.

2. Promise 기반 순차 프레임 캡처

   seek는 순차 처리로 이루어져야 합니다. `video.currentTime` 값은 하나만 존재하는데, 만약 `Promise.all`같은 병렬 처리를 시도하면 currentTime에는 뒤섞인 프레임이 저장되고, 썸네일 순서가 꼬이는 문제가 발생합니다.

   이 순차 처리 구현 방법은 두 가지 방법이 있습니다.
   ...
   - Promise 래핑 + `async/await`
     ...
     만약 `seeked`이벤트를 Promise로 래핑하면 Promise 객체를 반환하게 되고, await으로 작업 완료를 기다리게 할 수 있습니다. 덕분에 for문으로 간단하게 순차 처리 구현이 가능합니다.
...
썸네일 생성 작업에 긴 시간이 소요됩니다. 이 때 UX개선을 위해 현재는 로딩 스피너를 띄우고 있습니다.

다만, 로딩 스피너는 진행 상황은 사용자가 확인하지 못합니다. 같은 시간을 기다려도 진행 상황이 보이면 체감 대기 시간이 짧게 느껴지기 때문에, 점진적 렌더링으로 썸네일을 시각화하여 UX를 개선할 계획입니다.
